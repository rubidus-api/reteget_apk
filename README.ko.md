...
**ReteGet v0.4.0** (최신 릴리즈) 다운로드 — [apk (안드로이드 2.3+)](https://github.com/rubidus-api/reteget_apk/releases/download/v0.4.0/reteget-0.4.0.apk) · [릴리즈 노트](https://github.com/rubidus-api/reteget_apk/releases/tag/v0.4.0)
...
<p align="center">
  ...
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_template-0.4.0.png" alt="안드로이드 2.3의 ReteGet URL 템플릿, 버전 입력칸, 완성된 주소와 옵션" width="220">
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_presets-0.4.0.png" alt="안드로이드 2.3의 ReteGet 기본 rete 프리셋: Latest, 찾아볼 페이지, 체크섬, 서명자" width="220">
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_edit-0.4.0.png" alt="안드로이드 2.3에서 ReteGet 프리셋 편집: 이름, 주소 패턴, 찾아볼 페이지" width="220">
</p>
...
- **rete 시리즈 프리셋 기본 내장**: ReteGet, ReteClock, ReteKey(안드로이드 4.0+용, 그리고 안드로이드 9+용)가 GitHub 릴리즈용 버전 템플릿으로 들어 있고, 예상 SHA-256과 서명 키도 미리 채워져 있으며, GitHub 릴리즈 목록이 찾아볼 페이지로 들어 있어 **Latest**로 각 앱의 가장 새 버전을 받습니다. 기본 목록이 갱신되어도 직접 추가한 프리셋은 그대로 남습니다.
- **프리셋 이름 지정 및 다운로드 이력 관리**: 프리셋마다 이름(예: *ReteGet*, *ReteClock*)을 붙여 템플릿 주소끼리 쉽게 구분합니다. *Download* 옆의 **Save**는 위 주소를 프리셋으로 남깁니다. 각 프리셋은 이름, 주소, 마지막 다운로드 기록을 한 문단으로 이어 보여 주고, 윗줄의 짧은 버튼으로 다룹니다: **Use**(또는 항목 누르기)는 알려진 버전을 채워 주소창으로 불러오고, **Latest**(찾아볼 페이지가 있는 프리셋)는 그 페이지에서 찾은 가장 새 버전을 받고, **Edit**(또는 길게 누르기)는 이름·주소·찾아볼 페이지를 바꾸고, **Up**/**Dn**은 순서를 바꾸고, **Del**은 지웁니다. **Select all**과 **Delete selected**는 체크한 프리셋에 작동합니다.
- **찾아볼 페이지에서 최신 받기**: 프리셋에는 주소와 별도로 찾아볼 페이지를 등록할 수 있습니다. 디렉터리 목록, 다운로드 페이지, 또는 GitHub 릴리즈 API 같은 JSON 목록입니다. **Latest**는 그 페이지를 읽어 프리셋 주소에 맞는 주소를 모두 모으고(`{1}`은 버전 자리: 숫자로 시작하는 숫자·영문자·점·밑줄이라 `app-{1}.apk`는 `app-1.0-legacy.apk`를 잡지 않습니다. `*`는 경로 한 칸 안의 아무 글자), 윈도 탐색기처럼 정렬해(숫자는 값으로 비교하므로 0.3.10이 0.3.9 뒤) 가장 뒤의 것을 큐에 넣습니다. 이 정렬에서는 `1.0-beta`가 `1.0` 뒤에 오므로, 그런 이름을 허용하는 패턴은 시험판을 고를 수 있습니다. GitHub 릴리즈 페이지는 다운로드 링크를 나중에 불러오므로, GitHub에는 링크가 들어 있는 `https://api.github.com/repos/<소유자>/<저장소>/releases`를 쓰세요.
...
- **APK 설치와 묻지 않는 업그레이드**: `.apk` 파일 다운로드가 끝나면 시스템 설치 프로그램으로 설치할지 묻습니다. 안드로이드 5 이상은 PackageInstaller 세션으로(이때 ReteGet이 그 앱의 설치자로 기록됩니다), 그보다 오래된 버전은 `ACTION_VIEW`로 설치합니다. 받은 파일이 **설치된 앱의 업그레이드**이고(설치본과 같은 서명 키, 더 새 버전, 체크섬 경고 없음) *서명 키가 같으면 설치된 앱을 묻지 않고 업그레이드*가 켜져 있으면(기본값) ReteGet은 묻지 않고 바로 업그레이드를 시작합니다. 안드로이드 12 이상에서는 ReteGet이 그 앱을 설치했고 새 버전이 최근 안드로이드 API를 대상으로 하면 시스템도 확인 화면 없이 설치합니다(ReteKey는 해당, ReteClock과 ReteGet은 API 28 대상이라 시스템이 한 번 묻습니다). 오래된 안드로이드는 늘 한 번 누르는 확인 화면이 뜨고, 새 앱은 절대 확인 없이 설치하지 않습니다.
