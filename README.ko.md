...
- **향상된 프리셋 관리 및 다운로드 이력 보존**: 자주 쓰는 주소 목록이 화면 하단에 카드 형태로 배치되며, 각 프리셋마다 마지막 다운로드 일시, 파일 크기, 버전, 파일명, SHA-256 해시, 서명자/지문 메타데이터가 함께 보관됩니다. 카드를 터치하거나 `▲` 버튼으로 주소창에 즉시 불러오며, 인라인 주소 수정(`✎`), 순서 변경(`↑`/`↓`), 삭제 시 확인 팝업(`✕`), 다중 선택 체크박스 및 일괄 삭제 툴바를 지원합니다.
...
- **초경량 단일 Dex**: 최종 APK 크기 75 KB 미만, 외부 라이브러리 제로, 무거운 Gradle 없이 단일 덱스로 빌드됩니다.
