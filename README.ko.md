...
**ReteGet v0.4.1** (최신 릴리즈) 다운로드 — [apk (안드로이드 2.3+)](https://github.com/rubidus-api/reteget_apk/releases/download/v0.4.1/reteget-0.4.1.apk) · [릴리즈 노트](https://github.com/rubidus-api/reteget_apk/releases/tag/v0.4.1)
...
- **APK 설치와 빠른 업그레이드**: `.apk` 파일 다운로드가 끝나면 시스템 설치 프로그램으로 설치할지 묻습니다. 안드로이드 5 이상은 PackageInstaller 세션으로, 그보다 오래된 버전은 `ACTION_VIEW`로 설치합니다. 받은 파일이 **설치된 앱의 업그레이드**이고(설치본과 같은 서명 키, 더 새 버전, 체크섬 경고 없음) *서명 키가 같은 업그레이드는 묻지 않고 설치 화면 열기*가 켜져 있으면(기본값) ReteGet은 자기 질문을 건너뛰고 시스템 설치 화면을 바로 엽니다. 설치와 업그레이드는 언제나 시스템 화면에서 직접 확인하며, ReteGet은 안드로이드에 그 확인을 생략해 달라고 요청하지 않습니다.
