...
- **APK Installation and Quick Upgrades**: Once an `.apk` file finishes downloading, ReteGet offers to install it with the system installer: through a PackageInstaller session on Android 5 and newer, and through `ACTION_VIEW` on older versions. When the file **upgrades an installed app** (same signing key as the installed one, a newer version, no checksum warning) and *Upgrades with the same signing key: open the installer without asking first* is on (the default), ReteGet skips its own question and opens the system installer at once. Every install and upgrade is still confirmed by you on the system's screen; ReteGet never asks Android to skip it.
