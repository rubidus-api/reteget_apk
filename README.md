...
- **Enhanced Presets Management & History**: Frequently used endpoints and template URLs are preserved as touchable cards at the bottom with historical download metadata (filename, file size, version, timestamp, computed SHA-256 checksum, and signing certificate fingerprint/author). Tap any preset card to immediately load it into the URL bar (`▲`). Manage presets with inline editing (`✎`), reordering (`↑`/`↓`), delete confirmation (`✕`), multi-selection checkboxes, and batch deletion.
...
- **Ultra-Lightweight & Single-Dex**: Under 75 KB APK size, single dex file, zero third-party libraries, and built without Gradle.
