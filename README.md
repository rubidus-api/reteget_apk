...
**ReteGet v0.4.0** (latest release) download — [apk (Android 2.3+)](https://github.com/rubidus-api/reteget_apk/releases/download/v0.4.0/reteget-0.4.0.apk) · [release notes](https://github.com/rubidus-api/reteget_apk/releases/tag/v0.4.0)
...
<p align="center">
  ...
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_template-0.4.0.png" alt="ReteGet URL template with the version field, the resolved URL and the options on Android 2.3" width="220">
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_presets-0.4.0.png" alt="ReteGet built-in rete presets with Latest, their index pages, checksums and signers on Android 2.3" width="220">
  &nbsp;&nbsp;
  <img src="docs/screenshots/screenshot_edit-0.4.0.png" alt="Editing a ReteGet preset on Android 2.3: name, URL pattern and index page" width="220">
</p>
...
- **Rete Series Presets Built In**: ReteGet, ReteClock and ReteKey (the Android 4.0+ build, plus the Android 9+ build) come as ready-made version templates for GitHub Releases, with the expected SHA-256 and signing key already filled in, and with GitHub's release list as their index page, so **Latest** fetches the newest version of each. Your own presets are kept when the built-in list is updated.
- **Named Presets & History Management**: Give each preset a name (e.g. *ReteGet*, *ReteClock*) so template URLs are easy to tell apart. **Save** next to *Download* keeps the URL above as a preset. Each preset shows its name, URL and last download record in one wrapping paragraph, with short actions on top: **Use** (or tap the preset) loads it into the URL bar with the known version filled in, **Latest** (on a preset with an index page) downloads the newest version found there, **Edit** (or press and hold) changes the name, URL and index page, **Up**/**Dn** reorder, **Del** deletes; **Select all** and **Delete selected** work on the checked presets.
- **Latest From an Index Page**: A preset can name a page to look on besides its URL: a directory listing, a download page, or a JSON list such as GitHub's releases API. **Latest** reads that page, collects every address on it that fits the preset URL (where `{1}` stands for a version: digits, letters, dots and underscores starting with a digit, so `app-{1}.apk` does not take `app-1.0-legacy.apk`; `*` stands for any text within one path segment), sorts them the way Windows Explorer sorts names (numbers by value, so 0.3.10 comes after 0.3.9) and queues the last one. Note that in that order `1.0-beta` sorts after `1.0`, so a pattern that allows it would take a pre-release. GitHub's release pages load their download links later, so for GitHub use `https://api.github.com/repos/<owner>/<repo>/releases`, which lists them.
...
- **APK Installation and Upgrades Without Asking**: Once an `.apk` file finishes downloading, ReteGet offers to install it with the system installer: through a PackageInstaller session on Android 5 and newer (which also records ReteGet as the app's installer), and through `ACTION_VIEW` on older versions. When the file **upgrades an installed app** (same signing key as the installed one, a newer version, no checksum warning) and *Upgrade installed apps without asking* is on (the default), ReteGet skips its own question and starts the upgrade at once. On Android 12 and newer the system then installs it without its confirmation screen too, if ReteGet installed that app and the new version targets a recent Android API level (ReteKey does; ReteClock and ReteGet target API 28, so the system still asks once). Older Android always shows its one-tap confirmation; a new app is never installed without it.
